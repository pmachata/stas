...
fn main() {
	...
	state = state.drain(..).filter(|entry| !entry.history.is_empty()).collect();
		...
		.filter(|ref stat|
			ctmatch.iter().any(|ref pat|
					   pat.matches(&stat.name)))
	    {
		let key = CounterKey { ifname: ifname.clone(),
		...
		if let Some(pos) = state.iter().position(|ref hist| hist.key == key) {
		    ...
		    state.push(CounterHistory{ age: 0, key: key,
	...
	for entry in &state {
	    print!("{}{}|{}\t|{}\t|{}",
		   ...
		   entry.key.ifname, entry.key.ctname,
